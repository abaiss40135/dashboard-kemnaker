@extends('templates.admin-lte.sislap', [
    'title'          => 'LAPORAN HASIL KEGIATAN OPERASI KELADI SAGU',
    'route_template' => route('cartenz.keladi-sagu.template-excel'),
    'route_import'   => route('cartenz.keladi-sagu.import-excel'),
    'route_export'   => route('cartenz.keladi-sagu.export-excel'),
    'route_store'    => route('cartenz.keladi-sagu.store'),
    'route_search'   => route('cartenz.keladi-sagu.search'),
    'route_destroy'  => route('cartenz.keladi-sagu.destroy', 'id'),
                    ...
                    <td class="align-middle text-center">
                        ...
                        ${ haveApprovals && !item.need_approve ? `` : `
                            <button class="btn btn-edit btn-warning mb-1"
                                    ...
                                    onclick="insertValueToFormEdit('${route('cartenz.keladi-sagu.update', item.id)}', {
                                        daops      : '${item.daops}',
                                        satgas     : '${item.satgas}',
                                        jam        : '${item.jam}',
                        ...
                        <form action="${ route('cartenz.keladi-sagu.destroy', item.id) }" method="post" id="${ item.id }">
                            @method('delete')
                        @csrf
