@extends('templates.admin-lte.sislap', [
    'title'          => 'LAPORAN HASIL KEGIATAN OPERASI PI AJAR',
    'route_template' => route('cartenz.pi-ajar.template-excel'),
    'route_import'   => route('cartenz.pi-ajar.import-excel'),
    'route_export'   => route('cartenz.pi-ajar.export-excel'),
    'route_store'    => route('cartenz.pi-ajar.store'),
    'route_search'   => route('cartenz.pi-ajar.search'),
    'route_destroy'  => route('cartenz.pi-ajar.destroy', 'id'),
                    ...
                    <td class="align-middle text-center">
                        ...
                        ${ haveApprovals && !item.need_approve ? `` : `
                            <button class="btn btn-edit btn-warning mb-1"
                                    ...
                                    onclick="insertValueToFormEdit('${route('cartenz.pi-ajar.update', item.id)}', {
                                        daops      : '${item.daops}',
                                        satgas     : '${item.satgas}',
                                        jam        : '${item.jam}',
                        ...
                        <form action="${ route('cartenz.pi-ajar.destroy', item.id) }" method="post" id="${ item.id }">
                            @method('delete')
                        @csrf
