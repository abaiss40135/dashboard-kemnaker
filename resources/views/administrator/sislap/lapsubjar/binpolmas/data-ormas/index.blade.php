...
<div class="wrapper">
    <div class="content-wrapper">
        @component('components.admin.content-header')
            @slot('title', 'Laporan Data Organisasi Masyarakat')
        @endcomponent
        <section class="content">
            <div class="container-fluid">
                <div class="card">
                    @component('components.sislap.card-header')
                        @slot('template_excel', route('data-ormas.template-excel'))
                        @slot('import_excel', route('data-ormas.import-excel'))
                    @endcomponent
                    <div class="card-body">
                        @if(isset($laporan))
                        <div>
                            <h2 class="h4 text-center mb-3"><b>Preview Laporan Data Organisasi Masyarakat Binaan</b></h2>
                            <div class="table-responsive mb-4">
                                <form action="{{ route('data-ormas.store') }}"
                                      ...
                                      <table class="table table-hover table-bordered">
                                          <tbody>
                                                <tr class="text-center bg-primary">
                                                    ...
                                                    <th class="align-middle text-center">Bergerak</th>
                                                    <th class="align-middle text-center">Kebijakan</th>
                                              ...
                                              @foreach ($laporan[0] as $key => $item)
                                                    @if($key  !== 0)
                                                        <tr>
                                                            ...
                                                            <td><textarea class="form-control" name="laporan[{{ $key }}][bergerak]"
                                                                        id="laporan[{{ $key }}][bergerak]" rows="4" maxlength="255">{{ $item[8] ?? '-' }}</textarea></td>
                                                            <td><textarea class="form-control" name="laporan[{{ $key }}][kebijakan]"
                                                                        id="laporan[{{ $key }}][kebijakan]" rows="4" maxlength="255">{{ $item[9] ?? '-' }}</textarea></td>
                        ...
                        @else
                        <div>
                            <h2 class="h4 text-center"><b>Laporan Data Organisasi Masyarakat Binaan</b></h2>
                            <form action="{{ route('data-ormas.export-excel') }}"
                            ...
                            @component('components.sislap.form-search')
                                @slot('route', route('data-ormas.export-excel'))
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="#" method="post" id="form-edit" class="form row">
                        ...
                        <div class="form-group col-md-6">
                            <label for="bergerak">Bergerak</label>
                            <input type="text" id="bergerak" name="bergerak"
                                   class="form-control">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="kebijakan">Kebijakan</label>
                            <input type="text" id="kebijakan" name="kebijakan"
                                   class="form-control">
                        </div>
                        <div class="form-group col-md-6">
...
@section('customjs')
    ...
    <script>
            ...
            content: (item, rowNumber) => {
                ...
                let table = `
                    <tr ${ haveApprovals && item.approval.is_approve === false ? 'class="bg-orange"': ''} ${ haveApprovals ? 'data-widget="expandable-table" aria-expanded="false"' : '' }>
                        ...
                        <td class="align-middle text-center">${ item.bergerak }</td>
                        <td class="align-middle text-center">${ item.kebijakan }</td>
                        ...
                        @can('sislap_approval_approve')
                            ...
                            <form action="${ route('data-ormas.destroy', item.id) }" method="post" id="${ item.id }">
                                @method('delete')
                            @csrf
        ...
        const insertValueToFormEdit = ({
                ...
                duk_pembina, pengurus, bergerak, kebijakan, jumlah_anggota,
                keterangan
            }) => {
            ...
            elFormEdit.setAttribute('action', route('data-ormas.update', id))
            ...
            elFormEdit.querySelector('input[name="bergerak"]').value       = bergerak
            elFormEdit.querySelector('input[name="kebijakan"]').value      = kebijakan
