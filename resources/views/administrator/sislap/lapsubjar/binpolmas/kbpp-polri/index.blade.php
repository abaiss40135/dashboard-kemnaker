...
<div class="wrapper">
    <div class="content-wrapper">
        @component('components.admin.content-header')
            @slot('title', 'Laporan Data KBPP Polri')
        @endcomponent
        <section class="content">
            <div class="container-fluid">
                <div class="card">
                    @component('components.sislap.card-header')
                        @slot('template_excel', route('kbpp-polri.template-excel'))
                        @slot('import_excel', route('kbpp-polri.import-excel'))
                    @endcomponent
                    <div class="card-body">
                        @if(isset($laporan))
                        <div>
                            <h2 class="h4 text-center mb-3"><b>Preview Laporan Data KBPP Polri</b></h2>
                            <div class="table-responsive mb-4">
                                <form action="{{ route('kbpp-polri.store') }}"
                        ...
                        <div>
                            ...
                            <form action="{{ route('kbpp-polri.export-excel') }}"
                            ...
                            @component('components.sislap.form-search')
                                @slot('route', route('kbpp-polri.export-excel'))
...
@section('customjs')
    ...
    <script>
                ...
                let table = `
                    <tr ${ haveApprovals && item.approval.is_approve === false ? 'class="bg-orange"': ''} ${ haveApprovals ? 'data-widget="expandable-table" aria-expanded="false"' : '' }>
                        ...
                        @can('sislap_approval_approve')
                            ...
                            <form action="${ route('kbpp-polri.destroy', item.id) }" method="post" id="${ item.id }">
                                @method('delete')
                            @csrf
        ...
        const insertValueToFormEdit = ({
            ...
            elFormEdit.setAttribute('action', route('kbpp-polri.update', id))
