    ...
    <div class="wrapper">
        <div class="content-wrapper">
            @component('components.admin.content-header')
                @slot('title', 'Laporan Data Polsus Berdasarkan Kepemilikan KTA')
            @endcomponent
            <section class="content">
                <div class="container-fluid">
                    <div class="card">
                        <div class="card-body">
                            <div>
                                <h2 class="h4 text-center"><b>Data Polsus Berdasarkan Kepemilikan KTA</b></h2>
                                <form action="{{ route('data-kepemilikan-kta.export-excel') }}"
...
{{--                                                </button>--}}
                                ...
                                @component('components.sislap.form-search')
                                    @slot('route', route('data-kepemilikan-kta.export-excel'))
...
@section('customjs')
    ...
    <script>
        const listLaporan = new ComponentWithPagination({
            ...
            searchState: {
                url: route('data-kepemilikan-kta.search'),
                data: {}
            },
...
${ item[`${jenis_polsus}{{'_' . $attribute}}`] && '{{$attribute}}' != 'jml' ?
                            `<div class="d-flex justify-content-center" style="width: max-content">
                                        <span>${ item[`${jenis_polsus}{{'_' . $attribute}}`] }<span> ${`<button type="button" class="btn btn-info btn-sm" onclick="detailPolsus(this)" data-polsuswilayah="${jenis_polsus},{{$attribute}},${item.provinsi},${item.kotakab},all" class="btn btn-sm btn-info"><i class="fas fa-info-circle"></i></button>`}
                                    </div>`
...
${ {{"item.{$police}_{$attribute}"}} > 0 && '{{$attribute}}' != 'jml' ?
                    `<div class="d-flex justify-content-center" style="width: max-content">
                                                                <span class="mr-3">${ {{"item.{$police}_{$attribute}"}} }<span> ${ `<button type="button" class="btn btn-info btn-sm" onclick="detailPolsus(this)" data-polsuswilayah="{{$police}},{{$attribute}},${item.provinsi},${item.kotakab},all" class="btn btn-sm btn-info"><i class="fas fa-info-circle"></i></button>` }
                                                            </div>`
