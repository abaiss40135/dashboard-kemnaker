@extends('templates.admin-lte.admin', ['title' => 'Data amunisi polsus'])
@section('customcss')
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h2 class="h4 text-center"><b>Data Amunisi yang dimiliki Polsus</b></h2>
            @can('sipolsus_create')
        ...
        <div class="card-body">
            <form action="{{ route('data-amunisi.export-excel') }}" class="form" method="post">
                @csrf
            ...
            content: (item, rowNumber) => {
                let table = `
                    <tr>
                        ...
                        @foreach($polices as $police)
                @foreach($attributes as $attribute)
                <td class="align-middle">${ {{"item.{$police}_{$attribute}"}} }</td>
                            @endforeach
                @endforeach
                </tr>`;
                return table;
