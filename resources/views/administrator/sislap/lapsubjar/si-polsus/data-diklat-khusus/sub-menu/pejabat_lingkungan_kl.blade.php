    ...
    <div class="wrapper">
        <div class="content-wrapper">
            @component('components.admin.content-header')
                @slot('title', 'Laporan Data Polsus Diklat Khusus Pejabat di Lingkungan K/L')
            @endcomponent
            <section class="content">
                <div class="container-fluid">
                    <div class="card">
                        <div class="card-body">
                            <div>
                                <h2 class="h4 text-center"><b>Data Polsus Diklat Khusus Pejabat di Lingkungan K/L</b></h2>
                                <form action="{{ route('data-diklat-khusus.pejabat-lingkungan-kl.export-excel') }}"
...
{{--                                                    @endif--}}
                    ...
                    {{--                            </button>--}}
                                ...
                                @component('components.sislap.form-search')
                                    @slot('route', route('data-diklat-khusus.pejabat-lingkungan-kl.export-excel'))
                                    @slot('className', 'd-none')
                                @endcomponent

                                <button type="button" onclick="initView(this)" class="btn btn-success d-none mb-2 init-view-btn">Kembali ke Tampilan Awal</button>
...
@section('customjs')
    ...
    <script>
        const listLaporan = new ComponentWithPagination({
            ...
            searchState: {
                url: route('data-diklat-khusus.pejabat-lingkungan-kl.search'),
                data: {}
            },
                        ...
                        @forEach($attributes as $attribute)
                            <td class="align-middle">
                                ...
                                `<div class="d-flex justify-content-center" style="width: max-content">
                                        <span>${ item[`${jenis_polsus}{{'_' . $attribute}}`] }<span> ${`<button type="button" class="btn btn-info btn-sm" onclick="detailPolsus(this)" data-polsuswilayah="${jenis_polsus},{{$attribute}},${item.provinsi},${item.kotakab},khusus_pejabat_kl" class="btn btn-sm btn-info"><i class="fas fa-info-circle"></i></button>`}
                                    </div>`
                    ...
                    @foreach($polices as $police)
                        @foreach($attributes as $attribute)
                            <td class="align-middle">
                                ...
                                `<div class="d-flex justify-content-center" style="width: max-content">
                                                <span class="mr-3">${ {{"item.{$police}_{$attribute}"}} }<span> ${ `<button type="button" class="btn btn-info btn-sm" onclick="detailPolsus(this)" data-polsuswilayah="{{$police}},{{$attribute}},${item.provinsi},${item.kotakab},khusus_pejabat_kl" class="btn btn-sm btn-info"><i class="fas fa-info-circle"></i></button>` }
                                            </div>`
            ...
            completeEvent: () => {
                // change thead because of dynamic content
                const searchFilterKl = document.querySelector('select[name="filter_kl"]').value;
                if (searchFilterKl) {
