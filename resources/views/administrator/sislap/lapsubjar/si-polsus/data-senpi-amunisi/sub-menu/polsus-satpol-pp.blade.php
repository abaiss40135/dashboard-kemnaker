@extends('templates.admin-lte.admin', ['title' => 'Data Senpi dan Amunisi Milik Polsus Satpon PP'])
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h2 class="h4 text-center"><b>Data Senpi dan Amunisi yang dimiliki Polsus Satpol PP</b></h2>
            @if(!auth()->user()->haveRole('operator_polsus_polda') && can('sipolsus_create'))
                <div style="position: absolute; right: 0.8rem; top: 0.8rem;">
                    <a href="{{ route('input-data-senpi-amunisi.index', 'polsus-satpol-pp') }}">
        ...
        <div class="card-body">
            <form action="{{ route('data-senpi-amunisi.polsus-satpol-pp.export-excel') }}" class="form" method="post">
                @csrf
...
@section('customjs')
    ...
    <script>
        const listLaporan = new ComponentWithPagination({
            ...
            searchState: {
                url: route('data-senpi-amunisi.polsus-satpol-pp.search'),
                data: {}
            },
                        ...
                        <td class="align-middle text-center" rowspan="2">
                            <button onclick="editSenpiAmunisi({
                                ...
                                id_senpi: [${item.senpi.id_data}],
                                    ...
                                    <form action="${route('input-data-senpi-amunisi.destroy', ['polsus_satpol_pp', item.senpi.id_data, item.amunisi.id_data])}" method="post" id="form-delete">
                                        @csrf
                                        @method('delete')
