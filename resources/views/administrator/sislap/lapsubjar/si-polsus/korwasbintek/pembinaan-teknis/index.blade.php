@extends('templates.admin-lte.sislap-non-harian', [
    'title'           => 'LAPORAN KEGIATAN KORWASBINTEK PEMBINAAN TEKNIS POLSUS',
    'route_template'  => route('korwasbintek.pembinaan-teknis.template-excel'),
    'route_import'    => route('korwasbintek.pembinaan-teknis.import-excel'),
    'route_export'    => route('korwasbintek.pembinaan-teknis.export-excel'),
    'route_store'     => route('korwasbintek.pembinaan-teknis.store'),
    'route_search'    => route('korwasbintek.pembinaan-teknis.search'),
    'route_destroy'   => route('korwasbintek.pembinaan-teknis.destroy', 'id'),
    ...
    <table class="table table-hover table-bordered table-responsive" style="table-layout: fixed;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="form-control"
                           name="laporan[{{ $key }}][jml_peserta]"
                           value="{{ $item[5] }}" required>
                </td>
                <td>
                    <input type="text" class="form-control"
                           name="laporan[{{ $key }}][tempat_kegiatan]"
...
@section('form_edit')
    ...
    <div class="form-group col-sm-6">
        <label for="nrp" class="form-label">Jumlah Peserta</label>
        <input class="form-control" type="text" name="jml_peserta" id="jml_peserta" required>
    </div>
    <div class="form-group col-sm-6">
        <label for="nrp" class="form-label">Tempat Kegiatan</label>
        <input class="form-control" type="text" name="tempate_kegiatan" id="tempate_kegiatan" required>
    </div>
    <div class="form-group col-sm-6">
    ...
    <script>
        function createTableRow(item, rowNumber) {
            ...
            <tr ${ haveApprovals && item.approval.is_approve === false ? 'class="bg-orange"' : ''}
                ...
                <td class="text-center">${ item.tempat_kegiatan }</td>
                ...
                <td class="align-middle text-center">
                    ...
                    <button class="btn btn-edit btn-warning mb-1"
                            ...
                            onclick="insertValueToFormEdit('${ route('korwasbintek.pembinaan-teknis.update', item.id) }', {
                                polda          : '${item.polda}',
                                polres         : '${item.polres}',
                                bentuk_kegiatan: '${item.bentuk_kegiatan}',
                                jml_kegiatan   : '${item.jml_kegiatan}',
                                jml_peserta    : '${item.jml_peserta}',
                    ...
                    <form action="${ route('korwasbintek.pembinaan-teknis.destroy', item.id) }" method="post" id="${ item.id }">
                        @method('delete')
                        @csrf
