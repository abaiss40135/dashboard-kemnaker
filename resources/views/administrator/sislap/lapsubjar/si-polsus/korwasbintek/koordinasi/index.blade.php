@extends('templates.admin-lte.sislap-non-harian', [
    'title'           => 'LAPORAN KEGIATAN KORWASBINTEK KOORDINASI POLSUS',
    'route_template'  => route('korwasbintek.koordinasi.template-excel'),
    'route_import'    => route('korwasbintek.koordinasi.import-excel'),
    'route_export'    => route('korwasbintek.koordinasi.export-excel'),
    'route_store'     => route('korwasbintek.koordinasi.store'),
    'route_search'    => route('korwasbintek.koordinasi.search'),
    'route_destroy'   => route('korwasbintek.koordinasi.destroy', 'id'),
...
@section('table_preview')
    <table class="table table-hover table-bordered table-responsive" style="table-layout: fixed;">
        ...
        <tr>
            ...
            <th class="align-middle">Jumlah Pers yang Terlibat</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="form-control"
                           name="laporan[{{ $key }}][jml_pers_yang_terlibat]"
...
@section('form_edit')
    ...
    <div class="form-group col-sm-6">
        <label for="nrp" class="form-label">Jumlah Pers yang Terlibat</label>
        <input class="form-control" type="text" name="jml_pers_yang_terlibat" id="jml_pers_yang_terlibat" required>
    </div>
    <div class="form-group col-sm-6">
    ...
    <script>
        function createTableRow(item, rowNumber) {
            ...
            <tr ${ haveApprovals && item.approval.is_approve === false ? 'class="bg-orange"' : ''}
                ...
                <td class="text-center">${ item.jml_pers_yang_terlibat }</td>
                ...
                <td class="align-middle text-center">
                    ...
                    <button class="btn btn-edit btn-warning mb-1"
                            ...
                            onclick="insertValueToFormEdit('${ route('korwasbintek.koordinasi.update', item.id) }', {
                                polda          : '${item.polda}',
                                polres         : '${item.polres}',
                                bentuk_kegiatan: '${item.bentuk_kegiatan}',
                                jml_kegiatan   : '${item.jml_kegiatan}',
                                jml_pers_yang_terlibat: '${item.jml_pers_yang_terlibat}',
                                hasil          : '${item.hasil}',
                    ...
                    <form action="${ route('korwasbintek.koordinasi.destroy', item.id) }" method="post" id="${ item.id }">
                        @method('delete')
                        @csrf
