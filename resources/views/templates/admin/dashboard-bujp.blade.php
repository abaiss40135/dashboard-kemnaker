<aside class="main-sidebar sidebar-dark-primary elevation-4">
  ...
  <a href="{{ url('bujp') }}" class="brand-link text-center">
    <span class="logo-lg">
    ...
    <nav class="mt-2 mb-5">
      ...
      {{-- transaksi-satpam --}}
        <li class="nav-item mt-3 {{ request()->is('transaksi-satpam') ? 'menu-is-opening menu-open' : '' }}">
          ...
          <ul class="nav nav-treeview">
            <li class="nav-item">
              <a href="{{ url('bujp/transaksi-satpam/master-data-diri-satpam') }}" class="nav-link {{ request()->is('*master-data-diri-satpam*') ? 'active' : '' }}">
                <i class="far fa-{{ request()->is('*master-data-diri-satpam*') ? 'dot-circle' : 'circle' }} nav-icon"></i>
                <p>Master Data Diri Satpam</p>
              </a>
            </li>
            {{-- <li class="nav-item">
              <a href="{{ url('bujp/transaksi-satpam/rencana-pelatihan') }}" class="nav-link {{ request()->is('*rencana-pelatihan*') ? 'active' : '' }}">
...
<script>
  const redirect = () => {
    document.location.href ="{{ url('bujp') }}"
  }

  const toggleNav = (el) => {
    event.preventDefault()
    el.parentElement.classList.remove('menu-is-opening')
  }
</script>
