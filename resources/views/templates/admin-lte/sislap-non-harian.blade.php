    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <form action="{{ $route_store }}" method="POST" onclick="disableSubmitButtonTemporarily()" enctype="multipart/form-data">
                    @csrf
                    <div class="table-responsive">
                        @yield('table_preview')
                    </div>
            ...
            @if(roles(['operator_bagopsnalev_polres', 'operator_bagopsnalev_polda', 'operator_binpolmas_polres', 'operator_binpolmas_polda']) && ($approval_button ?? true))
            <button type="button" class="btn btn-primary" id="btn-ajukan-approval">Ajukan Approval ke {{
                roles(['operator_bagopsnalev_polres', 'operator_binpolmas_polres'])
                ? 'Polda'
                : 'Mabes' }}</button>
            <hr>
            @endif
            ...
            <div class="col-md-12 d-flex justify-content-end">
                <p id="count-laporan" class="mb-0"></p>
            </div>
            @endif
                ...
                <div class="modal-body">
                    <form action="#" method="post" id="form-edit" class="row" enctype="multipart/form-data">
                        ...
                        <div class="col-12 d-flex justify-content-end mt-2">
                            <div>
                                <button type="reset" class="btn btn-warning" data-toggle="modal" data-target="#modal_edit" data-dismiss="modal">Batal</button>
...
@section('customjs')
    ...
    <script>
        ...
        formSubmitApproval?.addEventListener('submit', (e) => {
            ...
            axios.post(route('approval-laporan.list-polres'), {
        ...
        @if(!empty($route_count))
        listLaporan.fetchCount = function () {
            axios.post('{{ $route_count }}', {
                polda: listLaporan.searchState.data.polda,
                search: listLaporan.searchState.data.search,
                start_date: listLaporan.searchState.data.start_date,
                end_date: listLaporan.searchState.data.end_date
            })
                .then((response) => response.data)
                .then((data) => {
                    document.getElementById('count-laporan').innerHTML = `Total: ${data}`
                })
        }
        listLaporan.fetchCount()
        @endif
        ...
        const insertValueToFormEdit = (route, fields) => {
                ...
                for (let item in fields) {
                    ...
                    if (el.tagName === 'SELECT') {
                        if (item == 'perorangan_kelompok' || item == 'gender') { // special case for perorangan_kelompok/keterangan and gender
                            $(`#${item}`).val(fields[item])
                        } else {
                            // empty option
                            $(el).empty();
                            // create option and selected the option
                            const optionEl = document.createElement('option')
                            optionEl.value = fields[item]
                            optionEl.text = fields[item]
