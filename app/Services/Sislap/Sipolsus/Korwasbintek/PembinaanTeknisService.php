...
class PembinaanTeknisService implements ExportInterface
{
    ...
    protected function getQuery($request, $withApproval = null)
    {
        ...
        ->when($search, function ($query) use ($search) {
                return $query->where('polda',  'ilike', "%$search%")
                    ...
                    ->orWhere('jml_peserta',   'ilike', "%$search%")
                    ->orWhere('tempat_kegiatan', 'ilike', "%$search%")
        ...
        ->when($polda, function ($query) use ($polda) {
            return $query->join('personel', 'personel.user_id', '=', 'kegiatan_korwasbintek_pembinaan_teknis.user_id')
