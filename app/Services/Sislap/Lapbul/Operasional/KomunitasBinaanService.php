...
class KomunitasBinaanService implements ExportInterface
{
    ...
    protected function getQuery($search)
    {
        ...
        ->when($search, function ($query, $search) {
            return $query->where(function ($query) use ($search) {
                $query->where('komunitas_binaan', 'ilike', '%'.$search.'%')
                ->orWhere('jumlah_komunitas_binaan', 'ilike', '%'.$search.'%')
                ->orWhere('jumlah_anggota', 'ilike', '%'.$search.'%')
                ->orWhere('peran_polri', 'ilike', '%'.$search.'%')
