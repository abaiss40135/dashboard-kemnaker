...
class SatkamlingService implements ExportInterface
{
        ...
        ->when($search, fn ($q) =>
            ...
            ->orwhere(fn ($q) =>
                $q->when(is_numeric($search), fn ($q) => 
                    $q->where('satkamling_aktif', $search)
                    ->orWhere('satkamling_pasif', $search)
                    ->orWhere('satkamling_jumlah', $search)
                    ->orWhere('revitalisasi_baru', $search)
                    ->orWhere('revitalisasi_lama', $search)
                    ->orWhere('revitalisasi_jumlah', $search)
                )
            )
        )
        ->when($polda, fn ($q) =>
