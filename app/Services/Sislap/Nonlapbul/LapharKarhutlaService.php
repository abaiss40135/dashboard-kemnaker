...
class LapharKarhutlaService implements ExportInterface
{
    /**
    ...
    protected function getQuery($request)
    {
        ...
        ->when($search, function ($query, $search) {
            return $query->where(function ($query) use ($search) {
                ...
                ->orWhere('himbauan', 'ilike', '%'.$search .'%')
                ->orWhere('fgd', 'ilike', '%'.$search .'%')
                ->orWhere('maklumat_kapolda', 'ilike', '%'.$search .'%')
