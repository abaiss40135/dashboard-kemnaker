...
class PolsusSatpolPpController extends Controller
{
    ...
    {
        $this->service = new DataPensiunPolsusService('polsus_satpol_pp');
    }

    public function index()
    {
        return view('administrator.sislap.lapsubjar.si-polsus.data-pensiun-polsus.sub-menu.polsus-satpol-pp');
    }

    public function exportExcel() {
        $template = new TemplateLaporan('polsus_satpol_pp');
        return Excel::download($template, 'Laporan Data Anggota Aktif dan Pensiun Polsus Satpon PP - '.now()->translatedFormat(config('app.long_date_without_day_format')).'.xlsx');
    }
