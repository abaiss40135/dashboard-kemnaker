...
class PolhutLhkController extends Controller
{
    ...
    {
        $this->service = new DataPensiunPolsusService('polhut_lhk');
    }

    public function index()
    {
        return view('administrator.sislap.lapsubjar.si-polsus.data-pensiun-polsus.sub-menu.polhut-lhk');
    }

    public function exportExcel() {
        $template = new TemplateLaporan('polhut_lhk');
        return Excel::download($template, 'Laporan Data Anggota Aktif dan Pensiun Polsus Polhut LHK - '.now()->translatedFormat(config('app.long_date_without_day_format')).'.xlsx');
    }
