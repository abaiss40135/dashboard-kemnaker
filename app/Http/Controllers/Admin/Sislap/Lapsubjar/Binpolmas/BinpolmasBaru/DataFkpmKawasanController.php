...
use App\Http\Requests\Administrator\Sislap\Lapsubjar\BinpolmasBaru\DataFkpmKawasan\StoreRequest;
...
class DataFkpmKawasanController extends Controller
{
    ...
    {
        return view('administrator.sislap.lapsubjar.binpolmas.data-fkpm-kawasan.index', [
    ...
    {
        return view("administrator.sislap.lapsubjar.binpolmas.data-fkpm-kawasan.create");
    }
    ...
    public function store(StoreRequest $request)
    {
        ...
        if (empty($kode_satuan)) {
            ...
            ])->redirectTo(route('data-fkpm-kawasan.index'));
        }
        ...
        return redirect()->route('data-fkpm-kawasan.index');
    }
    ...
    public function templateExcel()
    {
        ...
        return (new DataFkpmKawasanExport(true))
            ->download('FORMAT LAPORAN DATA FKPM KAWASAN'
                .$additionalName.' '
                .now()->format('Y-m-d')
                .'.xlsx'
            );
    ...
    public function importExcel(Request $request, Excel $excel)
    {
        ...
        if(count($data[0]) > 50) {
            ...
            foreach ($chunkData as $index => $chunk) {
                ...
                BulkStoreDataBinpolmas::dispatch($laporan, $user, $levels, $level, $kode_satuan, DataFkpm::class, 'kawasan');
            }
    ...
    public function exportExcel()
    {
        ...
        return (new DataFkpmKawasanExport())
            ->download('Laporan Data FKPM Kawasan '
               .$additionalNotes
               .'.xlsx'
            );
    }
    ...
    {
        $collection = $this->DataFkpmKawasanService->search($request);
